...
namespace BoilerTronicsObjects.Objects.FactoryLayerObjects {
	 ...
	 */
	public class LubePump : PumpObject, Runnable{ // Runnable

		static int layerSourceId = 14;

		public static Vector2I []atPosArr = {
			new Vector2I(0,1), // UP
			new Vector2I(1,1), // RIGHT
			new Vector2I(2,1), // DOWN
			new Vector2I(3,1)  // LEFT
		};

		public LubePump(int OGX, int OGY, Quality Q, Placeable.Direction D, int altTitle = 0) : base(OGX, OGY, layerSourceId + (int) Q, atPosArr[(int) D], Q, D,altTitle) {
			RegisterSteppable();
		}
		...
		public void Step()
		{
			...
			if (PG != null) PG.addLiquid(PipeGroup.LiquidType.Lube, 10 * (int) this.GetQuality());
		}
