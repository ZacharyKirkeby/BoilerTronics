...
public static partial class ProgramValidator
{
	...
	// Update PreprocessProgram to also return source line numbers
	public static (Dictionary<string, int> labels, List<string> validLines, List<int> sourceLineNumbers, List<(int, string)> errors)
		PreprocessProgram(string terminal)
	{
		...
		if (string.IsNullOrWhiteSpace(terminal))
			return (labels, validLines, sourceLineNumbers, errors);

		// Run validation first
		errors = ValidateProgram(terminal);

		// Build label map, valid lines, AND source line mapping
		string[] rawLines = terminal.Split('\n');
		...
		for (int i = 0; i < rawLines.Length; i++)
		{
			...
			// Add valid instruction line AND its source line number
			validLines.Add(trimmed);
			sourceLineNumbers.Add(i); // Record which line in the editor this came from
		}
